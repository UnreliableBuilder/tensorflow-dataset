    ...
    self.assertEqual(
        ...
        [
            ...
            _ShardSpec(0, '/bar.tfrecord-00000-of-00002',
                       '/bar_tfrecord-00000-of-00002_index.json', 4, [
                           shard_utils.FileInstruction(
            ...
            _ShardSpec(1, '/bar.tfrecord-00001-of-00002',
                       '/bar_tfrecord-00001-of-00002_index.json', 4, [
                           shard_utils.FileInstruction(
...
def _read_records(path, file_format=file_adapters.DEFAULT_FILE_FORMAT):
  ...
  paths = list(filter(lambda p: not p.endswith('_index.json'), paths))
  all_recs = []
  for fpath in paths:
    all_recs.append(
        list(
...
def _read_indices(path):
  ...
  for path in paths:
    ...
    parsed_proto = json_format.Parse(json_str, index_info_pb2.IndexInfo())
    all_indices.append(list(parsed_proto.record_positions))
...
class WriterTest(testing.TestCase):
  ...
  def test_write_tfrecord(self):
    ...
    index_path = os.path.join(self.tmp_dir, 'foo_tfrecord')
    to_write = [
        (1, b'a'), (2, b'b'),
        (3, b'c'),
        (4, b'd'), (5, b'e'),
    ...
    written_index_files, all_indices = _read_indices(index_path)
  ...
  def test_write_riegeli(self):
    ...
    index_path = os.path.join(self.tmp_dir, 'foo_riegeli')
    ...
    self.assertEqual(
        written_index_files,
        ['foo_riegeli-0000%s-of-00005_index.json' % i for i in range(5)])
